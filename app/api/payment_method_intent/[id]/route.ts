...
const partnerApiKeys: Record<string, string> = JSON.parse(
  ...
  export async function GET(
    ...
    const response = await axiosInstance.get(
        `${envData.baseUrl}/payment_method_intents/${id}`,
        {
          headers: {
            "x-api-key": apiKey,
      ...
      console.log("Payment method intent retrieved:", response.data);
      return NextResponse.json(response.data);
    } catch (error) {
      console.error('Error retrieving payment method intent:', error);
      return NextResponse.json(
        { error: 'Failed to retrieve payment method intent' },
        { status: 500 }
      );
    }
  }